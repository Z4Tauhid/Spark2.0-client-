...
function ValueCard({ emoji, title, desc, delay, inView }) {
  return (
    ...
    >
      ...
      <h3 className="font-body text-lg font-bold text-[#1a2744] mb-2">{title}</h3>
...
function TeamCard({ name, role, emoji, delay, inView }) {
  return (
    <div
      ...
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className="w-48 h-48 rounded-2xl bg-[#1c244b]/5 flex items-center justify-center text-3xl mx-auto mb-4">{emoji}</div>
      <p className="font-body font-bold text-[#1a2744] text-base mb-1">{name}</p>
...
export default function About() {
  ...
  return (
    <div className="overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
            ...
            <div className={`relative transition-all duration-700 ease-out ${missionInView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
              ...
              {/* accent badge */}
              <div className="absolute -bottom-5 -right-3 w-[80px] h-[80px] rounded-2xl bg-[#ff8000] flex items-center justify-center shadow-xl">
                <div className="text-center">
                  <p className="font-body text-xl font-bold text-white leading-none">5+</p>
            ...
            <div className={`transition-all duration-700 delay-200 ease-out ${missionInView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
              <p className="font-body text-[#ff8000] font-semibold text-xs tracking-widest uppercase mb-3">Our Mission</p>
              ...
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  <div key={s.label} className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                    <p className="font-body text-xl font-bold text-[#1a2744] leading-none">{s.value}</p>
      ...
      <section className="py-16 sm:py-24 bg-gray-50" ref={valuesRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-12 transition-all duration-700 ${valuesInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <p className="font-body text-[#ff8000] font-semibold text-xs tracking-widest uppercase mb-3">What We Stand For</p>
            <h2 className="font-body text-3xl sm:text-4xl font-bold text-[#1a2744]">Our Values</h2>
      ...
      <section className="py-16 sm:py-24 bg-white" ref={timelineRef}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-12 transition-all duration-700 ${timelineInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <p className="font-body text-[#ff8000] font-semibold text-xs tracking-widest uppercase mb-3">Our Journey</p>
            <h2 className="font-body text-3xl sm:text-4xl font-bold text-[#1a2744]">How We Got Here</h2>
          </div>
              ...
              {MILESTONES.map((m, i) => (
                ...
                >
                  ...
                  <div className="flex-shrink-0 w-[72px] sm:w-[88px] flex flex-col items-end sm:items-center pt-1">
                    <span className={`font-body font-bold text-sm ${i === MILESTONES.length - 1 ? 'text-[#ff8000]' : 'text-[#1a2744]'}`}>
                      {m.year}
                    </span>
                    {/* dot on the line */}
                    <div className={`hidden sm:flex mt-1.5 w-3 h-3 rounded-full border-2 flex-shrink-0 ${i === MILESTONES.length - 1 ? 'border-[#ff8000] bg-[#ff8000]' : 'border-[#1a2744] bg-white'}`} />
                  </div>
                  {/* content */}
      ...
      <section className="py-16 sm:py-24 bg-gray-50" ref={teamRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-12 transition-all duration-700 ${teamInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <p className="font-body text-[#ff8000] font-semibold text-xs tracking-widest uppercase mb-3">The People</p>
            <h2 className="font-body text-3xl sm:text-4xl font-bold text-[#1a2744] mb-3">Meet the Team</h2>
      ...
      {/* ── CTA ── */}
      <section className="py-16 sm:py-20 bg-[#1c244b] relative overflow-hidden" ref={ctaRef}>
        <div className="absolute inset-0 opacity-[0.05]"
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className={`transition-all duration-700 ${ctaInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <p className="font-body text-[#ff8000] font-semibold text-xs tracking-widest uppercase mb-4">Work With Us</p>
